...
class ExpertDataset(Dataset):
    ...
    def __init__(self, data_root, learning_type = LearningType.IMITATION):
        ...
        self._images = torch.zeros((self._length, 512, 512, 4), dtype = torch.float32)
        self._steer = torch.zeros((self._length, ), dtype = torch.float32)
        self._throttle = torch.zeros((self._length, ), dtype = torch.float32)
        self._brake = torch.zeros((self._length, ), dtype = torch.float32)
        self._speed = torch.zeros((self._length, ), dtype = torch.float32)
        self._command = torch.zeros((self._length, ), dtype = torch.float32)
        self._route_dist = torch.zeros((self._length, ), dtype = torch.float32)
        self._route_angle = torch.zeros((self._length, ), dtype = torch.float32)
        self._lane_dist = torch.zeros((self._length, ), dtype = torch.float32)
        self._lane_angle = torch.zeros((self._length, ), dtype = torch.float32)
        self._hazard = torch.zeros((self._length, ), dtype = torch.float32)
        self._hazard_dist = torch.zeros((self._length, ), dtype = torch.float32)
        self._tl_state = torch.zeros((self._length, ), dtype = torch.float32)
        self._tl_dist = torch.zeros((self._length, ), dtype = torch.float32)
        self._is_junction = torch.zeros((self._length, ), dtype = torch.float32)
        
        for k in range(len(rgb_files)):
            # Read images as a NumPy array

            
            self._images[k] = torch.from_numpy(np.array(Image.open(os.path.join(self.data_root, "rgb", rgb_files[k]))))
...
if __name__=="__main__":
    ...
    expert_dataset = ExpertDataset("./dataset/", learning_type = LearningType.IMITATION)
